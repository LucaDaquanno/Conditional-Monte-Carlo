...
# -*- coding: utf-8 -*-
"""
Created on Mon May 22 10:09:48 2023
...
vola_distribution=(epsilon).rolling(window=wndw).std().dropna()
port_mean_distribution=(x).rolling(window=wndw).mean().dropna()
port_vola_distribution=(x).rolling(window=wndw).std().dropna()
...
aux=pd.concat([-v_1x, v_2x],axis=1)
F=aux.T
F.index=['Exp_value_const','Volatility_const']
F.columns=p_0.index
